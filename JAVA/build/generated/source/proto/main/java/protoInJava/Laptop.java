// Generated by the protocol buffer compiler.  DO NOT EDIT!
// source: laptop_message.proto

package protoInJava;

/**
 * Protobuf type {@code techschool.pcbook.Laptop}
 */
public final class Laptop extends
    com.google.protobuf.GeneratedMessageV3 implements
    // @@protoc_insertion_point(message_implements:techschool.pcbook.Laptop)
    LaptopOrBuilder {
private static final long serialVersionUID = 0L;
  // Use Laptop.newBuilder() to construct.
  private Laptop(com.google.protobuf.GeneratedMessageV3.Builder<?> builder) {
    super(builder);
  }
  private Laptop() {
    id_ = "";
    brand_ = "";
    name_ = "";
    gpus_ = java.util.Collections.emptyList();
    storages_ = java.util.Collections.emptyList();
  }

  @java.lang.Override
  @SuppressWarnings({"unused"})
  protected java.lang.Object newInstance(
      UnusedPrivateParameter unused) {
    return new Laptop();
  }

  @java.lang.Override
  public final com.google.protobuf.UnknownFieldSet
  getUnknownFields() {
    return this.unknownFields;
  }
  public static final com.google.protobuf.Descriptors.Descriptor
      getDescriptor() {
    return protoInJava.LaptopMessage.internal_static_techschool_pcbook_Laptop_descriptor;
  }

  @java.lang.Override
  protected com.google.protobuf.GeneratedMessageV3.FieldAccessorTable
      internalGetFieldAccessorTable() {
    return protoInJava.LaptopMessage.internal_static_techschool_pcbook_Laptop_fieldAccessorTable
        .ensureFieldAccessorsInitialized(
            protoInJava.Laptop.class, protoInJava.Laptop.Builder.class);
  }

  private int weightCase_ = 0;
  private java.lang.Object weight_;
  public enum WeightCase
      implements com.google.protobuf.Internal.EnumLite,
          com.google.protobuf.AbstractMessage.InternalOneOfEnum {
    WEIGHT_KG(10),
    WEIGHT_LB(11),
    WEIGHT_NOT_SET(0);
    private final int value;
    private WeightCase(int value) {
      this.value = value;
    }
    /**
     * @param value The number of the enum to look for.
     * @return The enum associated with the given number.
     * @deprecated Use {@link #forNumber(int)} instead.
     */
    @java.lang.Deprecated
    public static WeightCase valueOf(int value) {
      return forNumber(value);
    }

    public static WeightCase forNumber(int value) {
      switch (value) {
        case 10: return WEIGHT_KG;
        case 11: return WEIGHT_LB;
        case 0: return WEIGHT_NOT_SET;
        default: return null;
      }
    }
    public int getNumber() {
      return this.value;
    }
  };

  public WeightCase
  getWeightCase() {
    return WeightCase.forNumber(
        weightCase_);
  }

  public static final int ID_FIELD_NUMBER = 1;
  private volatile java.lang.Object id_;
  /**
   * <code>string id = 1;</code>
   * @return The id.
   */
  @java.lang.Override
  public java.lang.String getId() {
    java.lang.Object ref = id_;
    if (ref instanceof java.lang.String) {
      return (java.lang.String) ref;
    } else {
      com.google.protobuf.ByteString bs = 
          (com.google.protobuf.ByteString) ref;
      java.lang.String s = bs.toStringUtf8();
      id_ = s;
      return s;
    }
  }
  /**
   * <code>string id = 1;</code>
   * @return The bytes for id.
   */
  @java.lang.Override
  public com.google.protobuf.ByteString
      getIdBytes() {
    java.lang.Object ref = id_;
    if (ref instanceof java.lang.String) {
      com.google.protobuf.ByteString b = 
          com.google.protobuf.ByteString.copyFromUtf8(
              (java.lang.String) ref);
      id_ = b;
      return b;
    } else {
      return (com.google.protobuf.ByteString) ref;
    }
  }

  public static final int BRAND_FIELD_NUMBER = 2;
  private volatile java.lang.Object brand_;
  /**
   * <code>string brand = 2;</code>
   * @return The brand.
   */
  @java.lang.Override
  public java.lang.String getBrand() {
    java.lang.Object ref = brand_;
    if (ref instanceof java.lang.String) {
      return (java.lang.String) ref;
    } else {
      com.google.protobuf.ByteString bs = 
          (com.google.protobuf.ByteString) ref;
      java.lang.String s = bs.toStringUtf8();
      brand_ = s;
      return s;
    }
  }
  /**
   * <code>string brand = 2;</code>
   * @return The bytes for brand.
   */
  @java.lang.Override
  public com.google.protobuf.ByteString
      getBrandBytes() {
    java.lang.Object ref = brand_;
    if (ref instanceof java.lang.String) {
      com.google.protobuf.ByteString b = 
          com.google.protobuf.ByteString.copyFromUtf8(
              (java.lang.String) ref);
      brand_ = b;
      return b;
    } else {
      return (com.google.protobuf.ByteString) ref;
    }
  }

  public static final int NAME_FIELD_NUMBER = 3;
  private volatile java.lang.Object name_;
  /**
   * <code>string name = 3;</code>
   * @return The name.
   */
  @java.lang.Override
  public java.lang.String getName() {
    java.lang.Object ref = name_;
    if (ref instanceof java.lang.String) {
      return (java.lang.String) ref;
    } else {
      com.google.protobuf.ByteString bs = 
          (com.google.protobuf.ByteString) ref;
      java.lang.String s = bs.toStringUtf8();
      name_ = s;
      return s;
    }
  }
  /**
   * <code>string name = 3;</code>
   * @return The bytes for name.
   */
  @java.lang.Override
  public com.google.protobuf.ByteString
      getNameBytes() {
    java.lang.Object ref = name_;
    if (ref instanceof java.lang.String) {
      com.google.protobuf.ByteString b = 
          com.google.protobuf.ByteString.copyFromUtf8(
              (java.lang.String) ref);
      name_ = b;
      return b;
    } else {
      return (com.google.protobuf.ByteString) ref;
    }
  }

  public static final int CPU_FIELD_NUMBER = 4;
  private protoInJava.CPU cpu_;
  /**
   * <code>.techschool.pcbook.CPU cpu = 4;</code>
   * @return Whether the cpu field is set.
   */
  @java.lang.Override
  public boolean hasCpu() {
    return cpu_ != null;
  }
  /**
   * <code>.techschool.pcbook.CPU cpu = 4;</code>
   * @return The cpu.
   */
  @java.lang.Override
  public protoInJava.CPU getCpu() {
    return cpu_ == null ? protoInJava.CPU.getDefaultInstance() : cpu_;
  }
  /**
   * <code>.techschool.pcbook.CPU cpu = 4;</code>
   */
  @java.lang.Override
  public protoInJava.CPUOrBuilder getCpuOrBuilder() {
    return getCpu();
  }

  public static final int RAM_FIELD_NUMBER = 5;
  private protoInJava.Memory ram_;
  /**
   * <code>.techschool.pcbook.Memory ram = 5;</code>
   * @return Whether the ram field is set.
   */
  @java.lang.Override
  public boolean hasRam() {
    return ram_ != null;
  }
  /**
   * <code>.techschool.pcbook.Memory ram = 5;</code>
   * @return The ram.
   */
  @java.lang.Override
  public protoInJava.Memory getRam() {
    return ram_ == null ? protoInJava.Memory.getDefaultInstance() : ram_;
  }
  /**
   * <code>.techschool.pcbook.Memory ram = 5;</code>
   */
  @java.lang.Override
  public protoInJava.MemoryOrBuilder getRamOrBuilder() {
    return getRam();
  }

  public static final int GPUS_FIELD_NUMBER = 6;
  private java.util.List<protoInJava.GPU> gpus_;
  /**
   * <code>repeated .techschool.pcbook.GPU gpus = 6;</code>
   */
  @java.lang.Override
  public java.util.List<protoInJava.GPU> getGpusList() {
    return gpus_;
  }
  /**
   * <code>repeated .techschool.pcbook.GPU gpus = 6;</code>
   */
  @java.lang.Override
  public java.util.List<? extends protoInJava.GPUOrBuilder> 
      getGpusOrBuilderList() {
    return gpus_;
  }
  /**
   * <code>repeated .techschool.pcbook.GPU gpus = 6;</code>
   */
  @java.lang.Override
  public int getGpusCount() {
    return gpus_.size();
  }
  /**
   * <code>repeated .techschool.pcbook.GPU gpus = 6;</code>
   */
  @java.lang.Override
  public protoInJava.GPU getGpus(int index) {
    return gpus_.get(index);
  }
  /**
   * <code>repeated .techschool.pcbook.GPU gpus = 6;</code>
   */
  @java.lang.Override
  public protoInJava.GPUOrBuilder getGpusOrBuilder(
      int index) {
    return gpus_.get(index);
  }

  public static final int STORAGES_FIELD_NUMBER = 7;
  private java.util.List<protoInJava.Storage> storages_;
  /**
   * <code>repeated .techschool.pcbook.Storage storages = 7;</code>
   */
  @java.lang.Override
  public java.util.List<protoInJava.Storage> getStoragesList() {
    return storages_;
  }
  /**
   * <code>repeated .techschool.pcbook.Storage storages = 7;</code>
   */
  @java.lang.Override
  public java.util.List<? extends protoInJava.StorageOrBuilder> 
      getStoragesOrBuilderList() {
    return storages_;
  }
  /**
   * <code>repeated .techschool.pcbook.Storage storages = 7;</code>
   */
  @java.lang.Override
  public int getStoragesCount() {
    return storages_.size();
  }
  /**
   * <code>repeated .techschool.pcbook.Storage storages = 7;</code>
   */
  @java.lang.Override
  public protoInJava.Storage getStorages(int index) {
    return storages_.get(index);
  }
  /**
   * <code>repeated .techschool.pcbook.Storage storages = 7;</code>
   */
  @java.lang.Override
  public protoInJava.StorageOrBuilder getStoragesOrBuilder(
      int index) {
    return storages_.get(index);
  }

  public static final int SCREEN_FIELD_NUMBER = 8;
  private protoInJava.Screen screen_;
  /**
   * <code>.techschool.pcbook.Screen screen = 8;</code>
   * @return Whether the screen field is set.
   */
  @java.lang.Override
  public boolean hasScreen() {
    return screen_ != null;
  }
  /**
   * <code>.techschool.pcbook.Screen screen = 8;</code>
   * @return The screen.
   */
  @java.lang.Override
  public protoInJava.Screen getScreen() {
    return screen_ == null ? protoInJava.Screen.getDefaultInstance() : screen_;
  }
  /**
   * <code>.techschool.pcbook.Screen screen = 8;</code>
   */
  @java.lang.Override
  public protoInJava.ScreenOrBuilder getScreenOrBuilder() {
    return getScreen();
  }

  public static final int KEYBOARD_FIELD_NUMBER = 9;
  private protoInJava.Keyboard keyboard_;
  /**
   * <code>.techschool.pcbook.Keyboard keyboard = 9;</code>
   * @return Whether the keyboard field is set.
   */
  @java.lang.Override
  public boolean hasKeyboard() {
    return keyboard_ != null;
  }
  /**
   * <code>.techschool.pcbook.Keyboard keyboard = 9;</code>
   * @return The keyboard.
   */
  @java.lang.Override
  public protoInJava.Keyboard getKeyboard() {
    return keyboard_ == null ? protoInJava.Keyboard.getDefaultInstance() : keyboard_;
  }
  /**
   * <code>.techschool.pcbook.Keyboard keyboard = 9;</code>
   */
  @java.lang.Override
  public protoInJava.KeyboardOrBuilder getKeyboardOrBuilder() {
    return getKeyboard();
  }

  public static final int WEIGHT_KG_FIELD_NUMBER = 10;
  /**
   * <code>double weight_kg = 10;</code>
   * @return Whether the weightKg field is set.
   */
  @java.lang.Override
  public boolean hasWeightKg() {
    return weightCase_ == 10;
  }
  /**
   * <code>double weight_kg = 10;</code>
   * @return The weightKg.
   */
  @java.lang.Override
  public double getWeightKg() {
    if (weightCase_ == 10) {
      return (java.lang.Double) weight_;
    }
    return 0D;
  }

  public static final int WEIGHT_LB_FIELD_NUMBER = 11;
  /**
   * <code>double weight_lb = 11;</code>
   * @return Whether the weightLb field is set.
   */
  @java.lang.Override
  public boolean hasWeightLb() {
    return weightCase_ == 11;
  }
  /**
   * <code>double weight_lb = 11;</code>
   * @return The weightLb.
   */
  @java.lang.Override
  public double getWeightLb() {
    if (weightCase_ == 11) {
      return (java.lang.Double) weight_;
    }
    return 0D;
  }

  public static final int PRICE_USD_FIELD_NUMBER = 12;
  private double priceUsd_;
  /**
   * <code>double price_usd = 12;</code>
   * @return The priceUsd.
   */
  @java.lang.Override
  public double getPriceUsd() {
    return priceUsd_;
  }

  public static final int RELEASE_YEAR_FIELD_NUMBER = 13;
  private int releaseYear_;
  /**
   * <code>uint32 release_year = 13;</code>
   * @return The releaseYear.
   */
  @java.lang.Override
  public int getReleaseYear() {
    return releaseYear_;
  }

  public static final int UPDATED_AT_FIELD_NUMBER = 14;
  private com.google.protobuf.Timestamp updatedAt_;
  /**
   * <code>.google.protobuf.Timestamp updated_at = 14;</code>
   * @return Whether the updatedAt field is set.
   */
  @java.lang.Override
  public boolean hasUpdatedAt() {
    return updatedAt_ != null;
  }
  /**
   * <code>.google.protobuf.Timestamp updated_at = 14;</code>
   * @return The updatedAt.
   */
  @java.lang.Override
  public com.google.protobuf.Timestamp getUpdatedAt() {
    return updatedAt_ == null ? com.google.protobuf.Timestamp.getDefaultInstance() : updatedAt_;
  }
  /**
   * <code>.google.protobuf.Timestamp updated_at = 14;</code>
   */
  @java.lang.Override
  public com.google.protobuf.TimestampOrBuilder getUpdatedAtOrBuilder() {
    return getUpdatedAt();
  }

  private byte memoizedIsInitialized = -1;
  @java.lang.Override
  public final boolean isInitialized() {
    byte isInitialized = memoizedIsInitialized;
    if (isInitialized == 1) return true;
    if (isInitialized == 0) return false;

    memoizedIsInitialized = 1;
    return true;
  }

  @java.lang.Override
  public void writeTo(com.google.protobuf.CodedOutputStream output)
                      throws java.io.IOException {
    if (!com.google.protobuf.GeneratedMessageV3.isStringEmpty(id_)) {
      com.google.protobuf.GeneratedMessageV3.writeString(output, 1, id_);
    }
    if (!com.google.protobuf.GeneratedMessageV3.isStringEmpty(brand_)) {
      com.google.protobuf.GeneratedMessageV3.writeString(output, 2, brand_);
    }
    if (!com.google.protobuf.GeneratedMessageV3.isStringEmpty(name_)) {
      com.google.protobuf.GeneratedMessageV3.writeString(output, 3, name_);
    }
    if (cpu_ != null) {
      output.writeMessage(4, getCpu());
    }
    if (ram_ != null) {
      output.writeMessage(5, getRam());
    }
    for (int i = 0; i < gpus_.size(); i++) {
      output.writeMessage(6, gpus_.get(i));
    }
    for (int i = 0; i < storages_.size(); i++) {
      output.writeMessage(7, storages_.get(i));
    }
    if (screen_ != null) {
      output.writeMessage(8, getScreen());
    }
    if (keyboard_ != null) {
      output.writeMessage(9, getKeyboard());
    }
    if (weightCase_ == 10) {
      output.writeDouble(
          10, (double)((java.lang.Double) weight_));
    }
    if (weightCase_ == 11) {
      output.writeDouble(
          11, (double)((java.lang.Double) weight_));
    }
    if (java.lang.Double.doubleToRawLongBits(priceUsd_) != 0) {
      output.writeDouble(12, priceUsd_);
    }
    if (releaseYear_ != 0) {
      output.writeUInt32(13, releaseYear_);
    }
    if (updatedAt_ != null) {
      output.writeMessage(14, getUpdatedAt());
    }
    getUnknownFields().writeTo(output);
  }

  @java.lang.Override
  public int getSerializedSize() {
    int size = memoizedSize;
    if (size != -1) return size;

    size = 0;
    if (!com.google.protobuf.GeneratedMessageV3.isStringEmpty(id_)) {
      size += com.google.protobuf.GeneratedMessageV3.computeStringSize(1, id_);
    }
    if (!com.google.protobuf.GeneratedMessageV3.isStringEmpty(brand_)) {
      size += com.google.protobuf.GeneratedMessageV3.computeStringSize(2, brand_);
    }
    if (!com.google.protobuf.GeneratedMessageV3.isStringEmpty(name_)) {
      size += com.google.protobuf.GeneratedMessageV3.computeStringSize(3, name_);
    }
    if (cpu_ != null) {
      size += com.google.protobuf.CodedOutputStream
        .computeMessageSize(4, getCpu());
    }
    if (ram_ != null) {
      size += com.google.protobuf.CodedOutputStream
        .computeMessageSize(5, getRam());
    }
    for (int i = 0; i < gpus_.size(); i++) {
      size += com.google.protobuf.CodedOutputStream
        .computeMessageSize(6, gpus_.get(i));
    }
    for (int i = 0; i < storages_.size(); i++) {
      size += com.google.protobuf.CodedOutputStream
        .computeMessageSize(7, storages_.get(i));
    }
    if (screen_ != null) {
      size += com.google.protobuf.CodedOutputStream
        .computeMessageSize(8, getScreen());
    }
    if (keyboard_ != null) {
      size += com.google.protobuf.CodedOutputStream
        .computeMessageSize(9, getKeyboard());
    }
    if (weightCase_ == 10) {
      size += com.google.protobuf.CodedOutputStream
        .computeDoubleSize(
            10, (double)((java.lang.Double) weight_));
    }
    if (weightCase_ == 11) {
      size += com.google.protobuf.CodedOutputStream
        .computeDoubleSize(
            11, (double)((java.lang.Double) weight_));
    }
    if (java.lang.Double.doubleToRawLongBits(priceUsd_) != 0) {
      size += com.google.protobuf.CodedOutputStream
        .computeDoubleSize(12, priceUsd_);
    }
    if (releaseYear_ != 0) {
      size += com.google.protobuf.CodedOutputStream
        .computeUInt32Size(13, releaseYear_);
    }
    if (updatedAt_ != null) {
      size += com.google.protobuf.CodedOutputStream
        .computeMessageSize(14, getUpdatedAt());
    }
    size += getUnknownFields().getSerializedSize();
    memoizedSize = size;
    return size;
  }

  @java.lang.Override
  public boolean equals(final java.lang.Object obj) {
    if (obj == this) {
     return true;
    }
    if (!(obj instanceof protoInJava.Laptop)) {
      return super.equals(obj);
    }
    protoInJava.Laptop other = (protoInJava.Laptop) obj;

    if (!getId()
        .equals(other.getId())) return false;
    if (!getBrand()
        .equals(other.getBrand())) return false;
    if (!getName()
        .equals(other.getName())) return false;
    if (hasCpu() != other.hasCpu()) return false;
    if (hasCpu()) {
      if (!getCpu()
          .equals(other.getCpu())) return false;
    }
    if (hasRam() != other.hasRam()) return false;
    if (hasRam()) {
      if (!getRam()
          .equals(other.getRam())) return false;
    }
    if (!getGpusList()
        .equals(other.getGpusList())) return false;
    if (!getStoragesList()
        .equals(other.getStoragesList())) return false;
    if (hasScreen() != other.hasScreen()) return false;
    if (hasScreen()) {
      if (!getScreen()
          .equals(other.getScreen())) return false;
    }
    if (hasKeyboard() != other.hasKeyboard()) return false;
    if (hasKeyboard()) {
      if (!getKeyboard()
          .equals(other.getKeyboard())) return false;
    }
    if (java.lang.Double.doubleToLongBits(getPriceUsd())
        != java.lang.Double.doubleToLongBits(
            other.getPriceUsd())) return false;
    if (getReleaseYear()
        != other.getReleaseYear()) return false;
    if (hasUpdatedAt() != other.hasUpdatedAt()) return false;
    if (hasUpdatedAt()) {
      if (!getUpdatedAt()
          .equals(other.getUpdatedAt())) return false;
    }
    if (!getWeightCase().equals(other.getWeightCase())) return false;
    switch (weightCase_) {
      case 10:
        if (java.lang.Double.doubleToLongBits(getWeightKg())
            != java.lang.Double.doubleToLongBits(
                other.getWeightKg())) return false;
        break;
      case 11:
        if (java.lang.Double.doubleToLongBits(getWeightLb())
            != java.lang.Double.doubleToLongBits(
                other.getWeightLb())) return false;
        break;
      case 0:
      default:
    }
    if (!getUnknownFields().equals(other.getUnknownFields())) return false;
    return true;
  }

  @java.lang.Override
  public int hashCode() {
    if (memoizedHashCode != 0) {
      return memoizedHashCode;
    }
    int hash = 41;
    hash = (19 * hash) + getDescriptor().hashCode();
    hash = (37 * hash) + ID_FIELD_NUMBER;
    hash = (53 * hash) + getId().hashCode();
    hash = (37 * hash) + BRAND_FIELD_NUMBER;
    hash = (53 * hash) + getBrand().hashCode();
    hash = (37 * hash) + NAME_FIELD_NUMBER;
    hash = (53 * hash) + getName().hashCode();
    if (hasCpu()) {
      hash = (37 * hash) + CPU_FIELD_NUMBER;
      hash = (53 * hash) + getCpu().hashCode();
    }
    if (hasRam()) {
      hash = (37 * hash) + RAM_FIELD_NUMBER;
      hash = (53 * hash) + getRam().hashCode();
    }
    if (getGpusCount() > 0) {
      hash = (37 * hash) + GPUS_FIELD_NUMBER;
      hash = (53 * hash) + getGpusList().hashCode();
    }
    if (getStoragesCount() > 0) {
      hash = (37 * hash) + STORAGES_FIELD_NUMBER;
      hash = (53 * hash) + getStoragesList().hashCode();
    }
    if (hasScreen()) {
      hash = (37 * hash) + SCREEN_FIELD_NUMBER;
      hash = (53 * hash) + getScreen().hashCode();
    }
    if (hasKeyboard()) {
      hash = (37 * hash) + KEYBOARD_FIELD_NUMBER;
      hash = (53 * hash) + getKeyboard().hashCode();
    }
    hash = (37 * hash) + PRICE_USD_FIELD_NUMBER;
    hash = (53 * hash) + com.google.protobuf.Internal.hashLong(
        java.lang.Double.doubleToLongBits(getPriceUsd()));
    hash = (37 * hash) + RELEASE_YEAR_FIELD_NUMBER;
    hash = (53 * hash) + getReleaseYear();
    if (hasUpdatedAt()) {
      hash = (37 * hash) + UPDATED_AT_FIELD_NUMBER;
      hash = (53 * hash) + getUpdatedAt().hashCode();
    }
    switch (weightCase_) {
      case 10:
        hash = (37 * hash) + WEIGHT_KG_FIELD_NUMBER;
        hash = (53 * hash) + com.google.protobuf.Internal.hashLong(
            java.lang.Double.doubleToLongBits(getWeightKg()));
        break;
      case 11:
        hash = (37 * hash) + WEIGHT_LB_FIELD_NUMBER;
        hash = (53 * hash) + com.google.protobuf.Internal.hashLong(
            java.lang.Double.doubleToLongBits(getWeightLb()));
        break;
      case 0:
      default:
    }
    hash = (29 * hash) + getUnknownFields().hashCode();
    memoizedHashCode = hash;
    return hash;
  }

  public static protoInJava.Laptop parseFrom(
      java.nio.ByteBuffer data)
      throws com.google.protobuf.InvalidProtocolBufferException {
    return PARSER.parseFrom(data);
  }
  public static protoInJava.Laptop parseFrom(
      java.nio.ByteBuffer data,
      com.google.protobuf.ExtensionRegistryLite extensionRegistry)
      throws com.google.protobuf.InvalidProtocolBufferException {
    return PARSER.parseFrom(data, extensionRegistry);
  }
  public static protoInJava.Laptop parseFrom(
      com.google.protobuf.ByteString data)
      throws com.google.protobuf.InvalidProtocolBufferException {
    return PARSER.parseFrom(data);
  }
  public static protoInJava.Laptop parseFrom(
      com.google.protobuf.ByteString data,
      com.google.protobuf.ExtensionRegistryLite extensionRegistry)
      throws com.google.protobuf.InvalidProtocolBufferException {
    return PARSER.parseFrom(data, extensionRegistry);
  }
  public static protoInJava.Laptop parseFrom(byte[] data)
      throws com.google.protobuf.InvalidProtocolBufferException {
    return PARSER.parseFrom(data);
  }
  public static protoInJava.Laptop parseFrom(
      byte[] data,
      com.google.protobuf.ExtensionRegistryLite extensionRegistry)
      throws com.google.protobuf.InvalidProtocolBufferException {
    return PARSER.parseFrom(data, extensionRegistry);
  }
  public static protoInJava.Laptop parseFrom(java.io.InputStream input)
      throws java.io.IOException {
    return com.google.protobuf.GeneratedMessageV3
        .parseWithIOException(PARSER, input);
  }
  public static protoInJava.Laptop parseFrom(
      java.io.InputStream input,
      com.google.protobuf.ExtensionRegistryLite extensionRegistry)
      throws java.io.IOException {
    return com.google.protobuf.GeneratedMessageV3
        .parseWithIOException(PARSER, input, extensionRegistry);
  }
  public static protoInJava.Laptop parseDelimitedFrom(java.io.InputStream input)
      throws java.io.IOException {
    return com.google.protobuf.GeneratedMessageV3
        .parseDelimitedWithIOException(PARSER, input);
  }
  public static protoInJava.Laptop parseDelimitedFrom(
      java.io.InputStream input,
      com.google.protobuf.ExtensionRegistryLite extensionRegistry)
      throws java.io.IOException {
    return com.google.protobuf.GeneratedMessageV3
        .parseDelimitedWithIOException(PARSER, input, extensionRegistry);
  }
  public static protoInJava.Laptop parseFrom(
      com.google.protobuf.CodedInputStream input)
      throws java.io.IOException {
    return com.google.protobuf.GeneratedMessageV3
        .parseWithIOException(PARSER, input);
  }
  public static protoInJava.Laptop parseFrom(
      com.google.protobuf.CodedInputStream input,
      com.google.protobuf.ExtensionRegistryLite extensionRegistry)
      throws java.io.IOException {
    return com.google.protobuf.GeneratedMessageV3
        .parseWithIOException(PARSER, input, extensionRegistry);
  }

  @java.lang.Override
  public Builder newBuilderForType() { return newBuilder(); }
  public static Builder newBuilder() {
    return DEFAULT_INSTANCE.toBuilder();
  }
  public static Builder newBuilder(protoInJava.Laptop prototype) {
    return DEFAULT_INSTANCE.toBuilder().mergeFrom(prototype);
  }
  @java.lang.Override
  public Builder toBuilder() {
    return this == DEFAULT_INSTANCE
        ? new Builder() : new Builder().mergeFrom(this);
  }

  @java.lang.Override
  protected Builder newBuilderForType(
      com.google.protobuf.GeneratedMessageV3.BuilderParent parent) {
    Builder builder = new Builder(parent);
    return builder;
  }
  /**
   * Protobuf type {@code techschool.pcbook.Laptop}
   */
  public static final class Builder extends
      com.google.protobuf.GeneratedMessageV3.Builder<Builder> implements
      // @@protoc_insertion_point(builder_implements:techschool.pcbook.Laptop)
      protoInJava.LaptopOrBuilder {
    public static final com.google.protobuf.Descriptors.Descriptor
        getDescriptor() {
      return protoInJava.LaptopMessage.internal_static_techschool_pcbook_Laptop_descriptor;
    }

    @java.lang.Override
    protected com.google.protobuf.GeneratedMessageV3.FieldAccessorTable
        internalGetFieldAccessorTable() {
      return protoInJava.LaptopMessage.internal_static_techschool_pcbook_Laptop_fieldAccessorTable
          .ensureFieldAccessorsInitialized(
              protoInJava.Laptop.class, protoInJava.Laptop.Builder.class);
    }

    // Construct using protoInJava.Laptop.newBuilder()
    private Builder() {

    }

    private Builder(
        com.google.protobuf.GeneratedMessageV3.BuilderParent parent) {
      super(parent);

    }
    @java.lang.Override
    public Builder clear() {
      super.clear();
      id_ = "";

      brand_ = "";

      name_ = "";

      if (cpuBuilder_ == null) {
        cpu_ = null;
      } else {
        cpu_ = null;
        cpuBuilder_ = null;
      }
      if (ramBuilder_ == null) {
        ram_ = null;
      } else {
        ram_ = null;
        ramBuilder_ = null;
      }
      if (gpusBuilder_ == null) {
        gpus_ = java.util.Collections.emptyList();
      } else {
        gpus_ = null;
        gpusBuilder_.clear();
      }
      bitField0_ = (bitField0_ & ~0x00000001);
      if (storagesBuilder_ == null) {
        storages_ = java.util.Collections.emptyList();
      } else {
        storages_ = null;
        storagesBuilder_.clear();
      }
      bitField0_ = (bitField0_ & ~0x00000002);
      if (screenBuilder_ == null) {
        screen_ = null;
      } else {
        screen_ = null;
        screenBuilder_ = null;
      }
      if (keyboardBuilder_ == null) {
        keyboard_ = null;
      } else {
        keyboard_ = null;
        keyboardBuilder_ = null;
      }
      priceUsd_ = 0D;

      releaseYear_ = 0;

      if (updatedAtBuilder_ == null) {
        updatedAt_ = null;
      } else {
        updatedAt_ = null;
        updatedAtBuilder_ = null;
      }
      weightCase_ = 0;
      weight_ = null;
      return this;
    }

    @java.lang.Override
    public com.google.protobuf.Descriptors.Descriptor
        getDescriptorForType() {
      return protoInJava.LaptopMessage.internal_static_techschool_pcbook_Laptop_descriptor;
    }

    @java.lang.Override
    public protoInJava.Laptop getDefaultInstanceForType() {
      return protoInJava.Laptop.getDefaultInstance();
    }

    @java.lang.Override
    public protoInJava.Laptop build() {
      protoInJava.Laptop result = buildPartial();
      if (!result.isInitialized()) {
        throw newUninitializedMessageException(result);
      }
      return result;
    }

    @java.lang.Override
    public protoInJava.Laptop buildPartial() {
      protoInJava.Laptop result = new protoInJava.Laptop(this);
      int from_bitField0_ = bitField0_;
      result.id_ = id_;
      result.brand_ = brand_;
      result.name_ = name_;
      if (cpuBuilder_ == null) {
        result.cpu_ = cpu_;
      } else {
        result.cpu_ = cpuBuilder_.build();
      }
      if (ramBuilder_ == null) {
        result.ram_ = ram_;
      } else {
        result.ram_ = ramBuilder_.build();
      }
      if (gpusBuilder_ == null) {
        if (((bitField0_ & 0x00000001) != 0)) {
          gpus_ = java.util.Collections.unmodifiableList(gpus_);
          bitField0_ = (bitField0_ & ~0x00000001);
        }
        result.gpus_ = gpus_;
      } else {
        result.gpus_ = gpusBuilder_.build();
      }
      if (storagesBuilder_ == null) {
        if (((bitField0_ & 0x00000002) != 0)) {
          storages_ = java.util.Collections.unmodifiableList(storages_);
          bitField0_ = (bitField0_ & ~0x00000002);
        }
        result.storages_ = storages_;
      } else {
        result.storages_ = storagesBuilder_.build();
      }
      if (screenBuilder_ == null) {
        result.screen_ = screen_;
      } else {
        result.screen_ = screenBuilder_.build();
      }
      if (keyboardBuilder_ == null) {
        result.keyboard_ = keyboard_;
      } else {
        result.keyboard_ = keyboardBuilder_.build();
      }
      if (weightCase_ == 10) {
        result.weight_ = weight_;
      }
      if (weightCase_ == 11) {
        result.weight_ = weight_;
      }
      result.priceUsd_ = priceUsd_;
      result.releaseYear_ = releaseYear_;
      if (updatedAtBuilder_ == null) {
        result.updatedAt_ = updatedAt_;
      } else {
        result.updatedAt_ = updatedAtBuilder_.build();
      }
      result.weightCase_ = weightCase_;
      onBuilt();
      return result;
    }

    @java.lang.Override
    public Builder clone() {
      return super.clone();
    }
    @java.lang.Override
    public Builder setField(
        com.google.protobuf.Descriptors.FieldDescriptor field,
        java.lang.Object value) {
      return super.setField(field, value);
    }
    @java.lang.Override
    public Builder clearField(
        com.google.protobuf.Descriptors.FieldDescriptor field) {
      return super.clearField(field);
    }
    @java.lang.Override
    public Builder clearOneof(
        com.google.protobuf.Descriptors.OneofDescriptor oneof) {
      return super.clearOneof(oneof);
    }
    @java.lang.Override
    public Builder setRepeatedField(
        com.google.protobuf.Descriptors.FieldDescriptor field,
        int index, java.lang.Object value) {
      return super.setRepeatedField(field, index, value);
    }
    @java.lang.Override
    public Builder addRepeatedField(
        com.google.protobuf.Descriptors.FieldDescriptor field,
        java.lang.Object value) {
      return super.addRepeatedField(field, value);
    }
    @java.lang.Override
    public Builder mergeFrom(com.google.protobuf.Message other) {
      if (other instanceof protoInJava.Laptop) {
        return mergeFrom((protoInJava.Laptop)other);
      } else {
        super.mergeFrom(other);
        return this;
      }
    }

    public Builder mergeFrom(protoInJava.Laptop other) {
      if (other == protoInJava.Laptop.getDefaultInstance()) return this;
      if (!other.getId().isEmpty()) {
        id_ = other.id_;
        onChanged();
      }
      if (!other.getBrand().isEmpty()) {
        brand_ = other.brand_;
        onChanged();
      }
      if (!other.getName().isEmpty()) {
        name_ = other.name_;
        onChanged();
      }
      if (other.hasCpu()) {
        mergeCpu(other.getCpu());
      }
      if (other.hasRam()) {
        mergeRam(other.getRam());
      }
      if (gpusBuilder_ == null) {
        if (!other.gpus_.isEmpty()) {
          if (gpus_.isEmpty()) {
            gpus_ = other.gpus_;
            bitField0_ = (bitField0_ & ~0x00000001);
          } else {
            ensureGpusIsMutable();
            gpus_.addAll(other.gpus_);
          }
          onChanged();
        }
      } else {
        if (!other.gpus_.isEmpty()) {
          if (gpusBuilder_.isEmpty()) {
            gpusBuilder_.dispose();
            gpusBuilder_ = null;
            gpus_ = other.gpus_;
            bitField0_ = (bitField0_ & ~0x00000001);
            gpusBuilder_ = 
              com.google.protobuf.GeneratedMessageV3.alwaysUseFieldBuilders ?
                 getGpusFieldBuilder() : null;
          } else {
            gpusBuilder_.addAllMessages(other.gpus_);
          }
        }
      }
      if (storagesBuilder_ == null) {
        if (!other.storages_.isEmpty()) {
          if (storages_.isEmpty()) {
            storages_ = other.storages_;
            bitField0_ = (bitField0_ & ~0x00000002);
          } else {
            ensureStoragesIsMutable();
            storages_.addAll(other.storages_);
          }
          onChanged();
        }
      } else {
        if (!other.storages_.isEmpty()) {
          if (storagesBuilder_.isEmpty()) {
            storagesBuilder_.dispose();
            storagesBuilder_ = null;
            storages_ = other.storages_;
            bitField0_ = (bitField0_ & ~0x00000002);
            storagesBuilder_ = 
              com.google.protobuf.GeneratedMessageV3.alwaysUseFieldBuilders ?
                 getStoragesFieldBuilder() : null;
          } else {
            storagesBuilder_.addAllMessages(other.storages_);
          }
        }
      }
      if (other.hasScreen()) {
        mergeScreen(other.getScreen());
      }
      if (other.hasKeyboard()) {
        mergeKeyboard(other.getKeyboard());
      }
      if (other.getPriceUsd() != 0D) {
        setPriceUsd(other.getPriceUsd());
      }
      if (other.getReleaseYear() != 0) {
        setReleaseYear(other.getReleaseYear());
      }
      if (other.hasUpdatedAt()) {
        mergeUpdatedAt(other.getUpdatedAt());
      }
      switch (other.getWeightCase()) {
        case WEIGHT_KG: {
          setWeightKg(other.getWeightKg());
          break;
        }
        case WEIGHT_LB: {
          setWeightLb(other.getWeightLb());
          break;
        }
        case WEIGHT_NOT_SET: {
          break;
        }
      }
      this.mergeUnknownFields(other.getUnknownFields());
      onChanged();
      return this;
    }

    @java.lang.Override
    public final boolean isInitialized() {
      return true;
    }

    @java.lang.Override
    public Builder mergeFrom(
        com.google.protobuf.CodedInputStream input,
        com.google.protobuf.ExtensionRegistryLite extensionRegistry)
        throws java.io.IOException {
      if (extensionRegistry == null) {
        throw new java.lang.NullPointerException();
      }
      try {
        boolean done = false;
        while (!done) {
          int tag = input.readTag();
          switch (tag) {
            case 0:
              done = true;
              break;
            case 10: {
              id_ = input.readStringRequireUtf8();

              break;
            } // case 10
            case 18: {
              brand_ = input.readStringRequireUtf8();

              break;
            } // case 18
            case 26: {
              name_ = input.readStringRequireUtf8();

              break;
            } // case 26
            case 34: {
              input.readMessage(
                  getCpuFieldBuilder().getBuilder(),
                  extensionRegistry);

              break;
            } // case 34
            case 42: {
              input.readMessage(
                  getRamFieldBuilder().getBuilder(),
                  extensionRegistry);

              break;
            } // case 42
            case 50: {
              protoInJava.GPU m =
                  input.readMessage(
                      protoInJava.GPU.parser(),
                      extensionRegistry);
              if (gpusBuilder_ == null) {
                ensureGpusIsMutable();
                gpus_.add(m);
              } else {
                gpusBuilder_.addMessage(m);
              }
              break;
            } // case 50
            case 58: {
              protoInJava.Storage m =
                  input.readMessage(
                      protoInJava.Storage.parser(),
                      extensionRegistry);
              if (storagesBuilder_ == null) {
                ensureStoragesIsMutable();
                storages_.add(m);
              } else {
                storagesBuilder_.addMessage(m);
              }
              break;
            } // case 58
            case 66: {
              input.readMessage(
                  getScreenFieldBuilder().getBuilder(),
                  extensionRegistry);

              break;
            } // case 66
            case 74: {
              input.readMessage(
                  getKeyboardFieldBuilder().getBuilder(),
                  extensionRegistry);

              break;
            } // case 74
            case 81: {
              weight_ = input.readDouble();
              weightCase_ = 10;
              break;
            } // case 81
            case 89: {
              weight_ = input.readDouble();
              weightCase_ = 11;
              break;
            } // case 89
            case 97: {
              priceUsd_ = input.readDouble();

              break;
            } // case 97
            case 104: {
              releaseYear_ = input.readUInt32();

              break;
            } // case 104
            case 114: {
              input.readMessage(
                  getUpdatedAtFieldBuilder().getBuilder(),
                  extensionRegistry);

              break;
            } // case 114
            default: {
              if (!super.parseUnknownField(input, extensionRegistry, tag)) {
                done = true; // was an endgroup tag
              }
              break;
            } // default:
          } // switch (tag)
        } // while (!done)
      } catch (com.google.protobuf.InvalidProtocolBufferException e) {
        throw e.unwrapIOException();
      } finally {
        onChanged();
      } // finally
      return this;
    }
    private int weightCase_ = 0;
    private java.lang.Object weight_;
    public WeightCase
        getWeightCase() {
      return WeightCase.forNumber(
          weightCase_);
    }

    public Builder clearWeight() {
      weightCase_ = 0;
      weight_ = null;
      onChanged();
      return this;
    }

    private int bitField0_;

    private java.lang.Object id_ = "";
    /**
     * <code>string id = 1;</code>
     * @return The id.
     */
    public java.lang.String getId() {
      java.lang.Object ref = id_;
      if (!(ref instanceof java.lang.String)) {
        com.google.protobuf.ByteString bs =
            (com.google.protobuf.ByteString) ref;
        java.lang.String s = bs.toStringUtf8();
        id_ = s;
        return s;
      } else {
        return (java.lang.String) ref;
      }
    }
    /**
     * <code>string id = 1;</code>
     * @return The bytes for id.
     */
    public com.google.protobuf.ByteString
        getIdBytes() {
      java.lang.Object ref = id_;
      if (ref instanceof String) {
        com.google.protobuf.ByteString b = 
            com.google.protobuf.ByteString.copyFromUtf8(
                (java.lang.String) ref);
        id_ = b;
        return b;
      } else {
        return (com.google.protobuf.ByteString) ref;
      }
    }
    /**
     * <code>string id = 1;</code>
     * @param value The id to set.
     * @return This builder for chaining.
     */
    public Builder setId(
        java.lang.String value) {
      if (value == null) {
    throw new NullPointerException();
  }
  
      id_ = value;
      onChanged();
      return this;
    }
    /**
     * <code>string id = 1;</code>
     * @return This builder for chaining.
     */
    public Builder clearId() {
      
      id_ = getDefaultInstance().getId();
      onChanged();
      return this;
    }
    /**
     * <code>string id = 1;</code>
     * @param value The bytes for id to set.
     * @return This builder for chaining.
     */
    public Builder setIdBytes(
        com.google.protobuf.ByteString value) {
      if (value == null) {
    throw new NullPointerException();
  }
  checkByteStringIsUtf8(value);
      
      id_ = value;
      onChanged();
      return this;
    }

    private java.lang.Object brand_ = "";
    /**
     * <code>string brand = 2;</code>
     * @return The brand.
     */
    public java.lang.String getBrand() {
      java.lang.Object ref = brand_;
      if (!(ref instanceof java.lang.String)) {
        com.google.protobuf.ByteString bs =
            (com.google.protobuf.ByteString) ref;
        java.lang.String s = bs.toStringUtf8();
        brand_ = s;
        return s;
      } else {
        return (java.lang.String) ref;
      }
    }
    /**
     * <code>string brand = 2;</code>
     * @return The bytes for brand.
     */
    public com.google.protobuf.ByteString
        getBrandBytes() {
      java.lang.Object ref = brand_;
      if (ref instanceof String) {
        com.google.protobuf.ByteString b = 
            com.google.protobuf.ByteString.copyFromUtf8(
                (java.lang.String) ref);
        brand_ = b;
        return b;
      } else {
        return (com.google.protobuf.ByteString) ref;
      }
    }
    /**
     * <code>string brand = 2;</code>
     * @param value The brand to set.
     * @return This builder for chaining.
     */
    public Builder setBrand(
        java.lang.String value) {
      if (value == null) {
    throw new NullPointerException();
  }
  
      brand_ = value;
      onChanged();
      return this;
    }
    /**
     * <code>string brand = 2;</code>
     * @return This builder for chaining.
     */
    public Builder clearBrand() {
      
      brand_ = getDefaultInstance().getBrand();
      onChanged();
      return this;
    }
    /**
     * <code>string brand = 2;</code>
     * @param value The bytes for brand to set.
     * @return This builder for chaining.
     */
    public Builder setBrandBytes(
        com.google.protobuf.ByteString value) {
      if (value == null) {
    throw new NullPointerException();
  }
  checkByteStringIsUtf8(value);
      
      brand_ = value;
      onChanged();
      return this;
    }

    private java.lang.Object name_ = "";
    /**
     * <code>string name = 3;</code>
     * @return The name.
     */
    public java.lang.String getName() {
      java.lang.Object ref = name_;
      if (!(ref instanceof java.lang.String)) {
        com.google.protobuf.ByteString bs =
            (com.google.protobuf.ByteString) ref;
        java.lang.String s = bs.toStringUtf8();
        name_ = s;
        return s;
      } else {
        return (java.lang.String) ref;
      }
    }
    /**
     * <code>string name = 3;</code>
     * @return The bytes for name.
     */
    public com.google.protobuf.ByteString
        getNameBytes() {
      java.lang.Object ref = name_;
      if (ref instanceof String) {
        com.google.protobuf.ByteString b = 
            com.google.protobuf.ByteString.copyFromUtf8(
                (java.lang.String) ref);
        name_ = b;
        return b;
      } else {
        return (com.google.protobuf.ByteString) ref;
      }
    }
    /**
     * <code>string name = 3;</code>
     * @param value The name to set.
     * @return This builder for chaining.
     */
    public Builder setName(
        java.lang.String value) {
      if (value == null) {
    throw new NullPointerException();
  }
  
      name_ = value;
      onChanged();
      return this;
    }
    /**
     * <code>string name = 3;</code>
     * @return This builder for chaining.
     */
    public Builder clearName() {
      
      name_ = getDefaultInstance().getName();
      onChanged();
      return this;
    }
    /**
     * <code>string name = 3;</code>
     * @param value The bytes for name to set.
     * @return This builder for chaining.
     */
    public Builder setNameBytes(
        com.google.protobuf.ByteString value) {
      if (value == null) {
    throw new NullPointerException();
  }
  checkByteStringIsUtf8(value);
      
      name_ = value;
      onChanged();
      return this;
    }

    private protoInJava.CPU cpu_;
    private com.google.protobuf.SingleFieldBuilderV3<
        protoInJava.CPU, protoInJava.CPU.Builder, protoInJava.CPUOrBuilder> cpuBuilder_;
    /**
     * <code>.techschool.pcbook.CPU cpu = 4;</code>
     * @return Whether the cpu field is set.
     */
    public boolean hasCpu() {
      return cpuBuilder_ != null || cpu_ != null;
    }
    /**
     * <code>.techschool.pcbook.CPU cpu = 4;</code>
     * @return The cpu.
     */
    public protoInJava.CPU getCpu() {
      if (cpuBuilder_ == null) {
        return cpu_ == null ? protoInJava.CPU.getDefaultInstance() : cpu_;
      } else {
        return cpuBuilder_.getMessage();
      }
    }
    /**
     * <code>.techschool.pcbook.CPU cpu = 4;</code>
     */
    public Builder setCpu(protoInJava.CPU value) {
      if (cpuBuilder_ == null) {
        if (value == null) {
          throw new NullPointerException();
        }
        cpu_ = value;
        onChanged();
      } else {
        cpuBuilder_.setMessage(value);
      }

      return this;
    }
    /**
     * <code>.techschool.pcbook.CPU cpu = 4;</code>
     */
    public Builder setCpu(
        protoInJava.CPU.Builder builderForValue) {
      if (cpuBuilder_ == null) {
        cpu_ = builderForValue.build();
        onChanged();
      } else {
        cpuBuilder_.setMessage(builderForValue.build());
      }

      return this;
    }
    /**
     * <code>.techschool.pcbook.CPU cpu = 4;</code>
     */
    public Builder mergeCpu(protoInJava.CPU value) {
      if (cpuBuilder_ == null) {
        if (cpu_ != null) {
          cpu_ =
            protoInJava.CPU.newBuilder(cpu_).mergeFrom(value).buildPartial();
        } else {
          cpu_ = value;
        }
        onChanged();
      } else {
        cpuBuilder_.mergeFrom(value);
      }

      return this;
    }
    /**
     * <code>.techschool.pcbook.CPU cpu = 4;</code>
     */
    public Builder clearCpu() {
      if (cpuBuilder_ == null) {
        cpu_ = null;
        onChanged();
      } else {
        cpu_ = null;
        cpuBuilder_ = null;
      }

      return this;
    }
    /**
     * <code>.techschool.pcbook.CPU cpu = 4;</code>
     */
    public protoInJava.CPU.Builder getCpuBuilder() {
      
      onChanged();
      return getCpuFieldBuilder().getBuilder();
    }
    /**
     * <code>.techschool.pcbook.CPU cpu = 4;</code>
     */
    public protoInJava.CPUOrBuilder getCpuOrBuilder() {
      if (cpuBuilder_ != null) {
        return cpuBuilder_.getMessageOrBuilder();
      } else {
        return cpu_ == null ?
            protoInJava.CPU.getDefaultInstance() : cpu_;
      }
    }
    /**
     * <code>.techschool.pcbook.CPU cpu = 4;</code>
     */
    private com.google.protobuf.SingleFieldBuilderV3<
        protoInJava.CPU, protoInJava.CPU.Builder, protoInJava.CPUOrBuilder> 
        getCpuFieldBuilder() {
      if (cpuBuilder_ == null) {
        cpuBuilder_ = new com.google.protobuf.SingleFieldBuilderV3<
            protoInJava.CPU, protoInJava.CPU.Builder, protoInJava.CPUOrBuilder>(
                getCpu(),
                getParentForChildren(),
                isClean());
        cpu_ = null;
      }
      return cpuBuilder_;
    }

    private protoInJava.Memory ram_;
    private com.google.protobuf.SingleFieldBuilderV3<
        protoInJava.Memory, protoInJava.Memory.Builder, protoInJava.MemoryOrBuilder> ramBuilder_;
    /**
     * <code>.techschool.pcbook.Memory ram = 5;</code>
     * @return Whether the ram field is set.
     */
    public boolean hasRam() {
      return ramBuilder_ != null || ram_ != null;
    }
    /**
     * <code>.techschool.pcbook.Memory ram = 5;</code>
     * @return The ram.
     */
    public protoInJava.Memory getRam() {
      if (ramBuilder_ == null) {
        return ram_ == null ? protoInJava.Memory.getDefaultInstance() : ram_;
      } else {
        return ramBuilder_.getMessage();
      }
    }
    /**
     * <code>.techschool.pcbook.Memory ram = 5;</code>
     */
    public Builder setRam(protoInJava.Memory value) {
      if (ramBuilder_ == null) {
        if (value == null) {
          throw new NullPointerException();
        }
        ram_ = value;
        onChanged();
      } else {
        ramBuilder_.setMessage(value);
      }

      return this;
    }
    /**
     * <code>.techschool.pcbook.Memory ram = 5;</code>
     */
    public Builder setRam(
        protoInJava.Memory.Builder builderForValue) {
      if (ramBuilder_ == null) {
        ram_ = builderForValue.build();
        onChanged();
      } else {
        ramBuilder_.setMessage(builderForValue.build());
      }

      return this;
    }
    /**
     * <code>.techschool.pcbook.Memory ram = 5;</code>
     */
    public Builder mergeRam(protoInJava.Memory value) {
      if (ramBuilder_ == null) {
        if (ram_ != null) {
          ram_ =
            protoInJava.Memory.newBuilder(ram_).mergeFrom(value).buildPartial();
        } else {
          ram_ = value;
        }
        onChanged();
      } else {
        ramBuilder_.mergeFrom(value);
      }

      return this;
    }
    /**
     * <code>.techschool.pcbook.Memory ram = 5;</code>
     */
    public Builder clearRam() {
      if (ramBuilder_ == null) {
        ram_ = null;
        onChanged();
      } else {
        ram_ = null;
        ramBuilder_ = null;
      }

      return this;
    }
    /**
     * <code>.techschool.pcbook.Memory ram = 5;</code>
     */
    public protoInJava.Memory.Builder getRamBuilder() {
      
      onChanged();
      return getRamFieldBuilder().getBuilder();
    }
    /**
     * <code>.techschool.pcbook.Memory ram = 5;</code>
     */
    public protoInJava.MemoryOrBuilder getRamOrBuilder() {
      if (ramBuilder_ != null) {
        return ramBuilder_.getMessageOrBuilder();
      } else {
        return ram_ == null ?
            protoInJava.Memory.getDefaultInstance() : ram_;
      }
    }
    /**
     * <code>.techschool.pcbook.Memory ram = 5;</code>
     */
    private com.google.protobuf.SingleFieldBuilderV3<
        protoInJava.Memory, protoInJava.Memory.Builder, protoInJava.MemoryOrBuilder> 
        getRamFieldBuilder() {
      if (ramBuilder_ == null) {
        ramBuilder_ = new com.google.protobuf.SingleFieldBuilderV3<
            protoInJava.Memory, protoInJava.Memory.Builder, protoInJava.MemoryOrBuilder>(
                getRam(),
                getParentForChildren(),
                isClean());
        ram_ = null;
      }
      return ramBuilder_;
    }

    private java.util.List<protoInJava.GPU> gpus_ =
      java.util.Collections.emptyList();
    private void ensureGpusIsMutable() {
      if (!((bitField0_ & 0x00000001) != 0)) {
        gpus_ = new java.util.ArrayList<protoInJava.GPU>(gpus_);
        bitField0_ |= 0x00000001;
       }
    }

    private com.google.protobuf.RepeatedFieldBuilderV3<
        protoInJava.GPU, protoInJava.GPU.Builder, protoInJava.GPUOrBuilder> gpusBuilder_;

    /**
     * <code>repeated .techschool.pcbook.GPU gpus = 6;</code>
     */
    public java.util.List<protoInJava.GPU> getGpusList() {
      if (gpusBuilder_ == null) {
        return java.util.Collections.unmodifiableList(gpus_);
      } else {
        return gpusBuilder_.getMessageList();
      }
    }
    /**
     * <code>repeated .techschool.pcbook.GPU gpus = 6;</code>
     */
    public int getGpusCount() {
      if (gpusBuilder_ == null) {
        return gpus_.size();
      } else {
        return gpusBuilder_.getCount();
      }
    }
    /**
     * <code>repeated .techschool.pcbook.GPU gpus = 6;</code>
     */
    public protoInJava.GPU getGpus(int index) {
      if (gpusBuilder_ == null) {
        return gpus_.get(index);
      } else {
        return gpusBuilder_.getMessage(index);
      }
    }
    /**
     * <code>repeated .techschool.pcbook.GPU gpus = 6;</code>
     */
    public Builder setGpus(
        int index, protoInJava.GPU value) {
      if (gpusBuilder_ == null) {
        if (value == null) {
          throw new NullPointerException();
        }
        ensureGpusIsMutable();
        gpus_.set(index, value);
        onChanged();
      } else {
        gpusBuilder_.setMessage(index, value);
      }
      return this;
    }
    /**
     * <code>repeated .techschool.pcbook.GPU gpus = 6;</code>
     */
    public Builder setGpus(
        int index, protoInJava.GPU.Builder builderForValue) {
      if (gpusBuilder_ == null) {
        ensureGpusIsMutable();
        gpus_.set(index, builderForValue.build());
        onChanged();
      } else {
        gpusBuilder_.setMessage(index, builderForValue.build());
      }
      return this;
    }
    /**
     * <code>repeated .techschool.pcbook.GPU gpus = 6;</code>
     */
    public Builder addGpus(protoInJava.GPU value) {
      if (gpusBuilder_ == null) {
        if (value == null) {
          throw new NullPointerException();
        }
        ensureGpusIsMutable();
        gpus_.add(value);
        onChanged();
      } else {
        gpusBuilder_.addMessage(value);
      }
      return this;
    }
    /**
     * <code>repeated .techschool.pcbook.GPU gpus = 6;</code>
     */
    public Builder addGpus(
        int index, protoInJava.GPU value) {
      if (gpusBuilder_ == null) {
        if (value == null) {
          throw new NullPointerException();
        }
        ensureGpusIsMutable();
        gpus_.add(index, value);
        onChanged();
      } else {
        gpusBuilder_.addMessage(index, value);
      }
      return this;
    }
    /**
     * <code>repeated .techschool.pcbook.GPU gpus = 6;</code>
     */
    public Builder addGpus(
        protoInJava.GPU.Builder builderForValue) {
      if (gpusBuilder_ == null) {
        ensureGpusIsMutable();
        gpus_.add(builderForValue.build());
        onChanged();
      } else {
        gpusBuilder_.addMessage(builderForValue.build());
      }
      return this;
    }
    /**
     * <code>repeated .techschool.pcbook.GPU gpus = 6;</code>
     */
    public Builder addGpus(
        int index, protoInJava.GPU.Builder builderForValue) {
      if (gpusBuilder_ == null) {
        ensureGpusIsMutable();
        gpus_.add(index, builderForValue.build());
        onChanged();
      } else {
        gpusBuilder_.addMessage(index, builderForValue.build());
      }
      return this;
    }
    /**
     * <code>repeated .techschool.pcbook.GPU gpus = 6;</code>
     */
    public Builder addAllGpus(
        java.lang.Iterable<? extends protoInJava.GPU> values) {
      if (gpusBuilder_ == null) {
        ensureGpusIsMutable();
        com.google.protobuf.AbstractMessageLite.Builder.addAll(
            values, gpus_);
        onChanged();
      } else {
        gpusBuilder_.addAllMessages(values);
      }
      return this;
    }
    /**
     * <code>repeated .techschool.pcbook.GPU gpus = 6;</code>
     */
    public Builder clearGpus() {
      if (gpusBuilder_ == null) {
        gpus_ = java.util.Collections.emptyList();
        bitField0_ = (bitField0_ & ~0x00000001);
        onChanged();
      } else {
        gpusBuilder_.clear();
      }
      return this;
    }
    /**
     * <code>repeated .techschool.pcbook.GPU gpus = 6;</code>
     */
    public Builder removeGpus(int index) {
      if (gpusBuilder_ == null) {
        ensureGpusIsMutable();
        gpus_.remove(index);
        onChanged();
      } else {
        gpusBuilder_.remove(index);
      }
      return this;
    }
    /**
     * <code>repeated .techschool.pcbook.GPU gpus = 6;</code>
     */
    public protoInJava.GPU.Builder getGpusBuilder(
        int index) {
      return getGpusFieldBuilder().getBuilder(index);
    }
    /**
     * <code>repeated .techschool.pcbook.GPU gpus = 6;</code>
     */
    public protoInJava.GPUOrBuilder getGpusOrBuilder(
        int index) {
      if (gpusBuilder_ == null) {
        return gpus_.get(index);  } else {
        return gpusBuilder_.getMessageOrBuilder(index);
      }
    }
    /**
     * <code>repeated .techschool.pcbook.GPU gpus = 6;</code>
     */
    public java.util.List<? extends protoInJava.GPUOrBuilder> 
         getGpusOrBuilderList() {
      if (gpusBuilder_ != null) {
        return gpusBuilder_.getMessageOrBuilderList();
      } else {
        return java.util.Collections.unmodifiableList(gpus_);
      }
    }
    /**
     * <code>repeated .techschool.pcbook.GPU gpus = 6;</code>
     */
    public protoInJava.GPU.Builder addGpusBuilder() {
      return getGpusFieldBuilder().addBuilder(
          protoInJava.GPU.getDefaultInstance());
    }
    /**
     * <code>repeated .techschool.pcbook.GPU gpus = 6;</code>
     */
    public protoInJava.GPU.Builder addGpusBuilder(
        int index) {
      return getGpusFieldBuilder().addBuilder(
          index, protoInJava.GPU.getDefaultInstance());
    }
    /**
     * <code>repeated .techschool.pcbook.GPU gpus = 6;</code>
     */
    public java.util.List<protoInJava.GPU.Builder> 
         getGpusBuilderList() {
      return getGpusFieldBuilder().getBuilderList();
    }
    private com.google.protobuf.RepeatedFieldBuilderV3<
        protoInJava.GPU, protoInJava.GPU.Builder, protoInJava.GPUOrBuilder> 
        getGpusFieldBuilder() {
      if (gpusBuilder_ == null) {
        gpusBuilder_ = new com.google.protobuf.RepeatedFieldBuilderV3<
            protoInJava.GPU, protoInJava.GPU.Builder, protoInJava.GPUOrBuilder>(
                gpus_,
                ((bitField0_ & 0x00000001) != 0),
                getParentForChildren(),
                isClean());
        gpus_ = null;
      }
      return gpusBuilder_;
    }

    private java.util.List<protoInJava.Storage> storages_ =
      java.util.Collections.emptyList();
    private void ensureStoragesIsMutable() {
      if (!((bitField0_ & 0x00000002) != 0)) {
        storages_ = new java.util.ArrayList<protoInJava.Storage>(storages_);
        bitField0_ |= 0x00000002;
       }
    }

    private com.google.protobuf.RepeatedFieldBuilderV3<
        protoInJava.Storage, protoInJava.Storage.Builder, protoInJava.StorageOrBuilder> storagesBuilder_;

    /**
     * <code>repeated .techschool.pcbook.Storage storages = 7;</code>
     */
    public java.util.List<protoInJava.Storage> getStoragesList() {
      if (storagesBuilder_ == null) {
        return java.util.Collections.unmodifiableList(storages_);
      } else {
        return storagesBuilder_.getMessageList();
      }
    }
    /**
     * <code>repeated .techschool.pcbook.Storage storages = 7;</code>
     */
    public int getStoragesCount() {
      if (storagesBuilder_ == null) {
        return storages_.size();
      } else {
        return storagesBuilder_.getCount();
      }
    }
    /**
     * <code>repeated .techschool.pcbook.Storage storages = 7;</code>
     */
    public protoInJava.Storage getStorages(int index) {
      if (storagesBuilder_ == null) {
        return storages_.get(index);
      } else {
        return storagesBuilder_.getMessage(index);
      }
    }
    /**
     * <code>repeated .techschool.pcbook.Storage storages = 7;</code>
     */
    public Builder setStorages(
        int index, protoInJava.Storage value) {
      if (storagesBuilder_ == null) {
        if (value == null) {
          throw new NullPointerException();
        }
        ensureStoragesIsMutable();
        storages_.set(index, value);
        onChanged();
      } else {
        storagesBuilder_.setMessage(index, value);
      }
      return this;
    }
    /**
     * <code>repeated .techschool.pcbook.Storage storages = 7;</code>
     */
    public Builder setStorages(
        int index, protoInJava.Storage.Builder builderForValue) {
      if (storagesBuilder_ == null) {
        ensureStoragesIsMutable();
        storages_.set(index, builderForValue.build());
        onChanged();
      } else {
        storagesBuilder_.setMessage(index, builderForValue.build());
      }
      return this;
    }
    /**
     * <code>repeated .techschool.pcbook.Storage storages = 7;</code>
     */
    public Builder addStorages(protoInJava.Storage value) {
      if (storagesBuilder_ == null) {
        if (value == null) {
          throw new NullPointerException();
        }
        ensureStoragesIsMutable();
        storages_.add(value);
        onChanged();
      } else {
        storagesBuilder_.addMessage(value);
      }
      return this;
    }
    /**
     * <code>repeated .techschool.pcbook.Storage storages = 7;</code>
     */
    public Builder addStorages(
        int index, protoInJava.Storage value) {
      if (storagesBuilder_ == null) {
        if (value == null) {
          throw new NullPointerException();
        }
        ensureStoragesIsMutable();
        storages_.add(index, value);
        onChanged();
      } else {
        storagesBuilder_.addMessage(index, value);
      }
      return this;
    }
    /**
     * <code>repeated .techschool.pcbook.Storage storages = 7;</code>
     */
    public Builder addStorages(
        protoInJava.Storage.Builder builderForValue) {
      if (storagesBuilder_ == null) {
        ensureStoragesIsMutable();
        storages_.add(builderForValue.build());
        onChanged();
      } else {
        storagesBuilder_.addMessage(builderForValue.build());
      }
      return this;
    }
    /**
     * <code>repeated .techschool.pcbook.Storage storages = 7;</code>
     */
    public Builder addStorages(
        int index, protoInJava.Storage.Builder builderForValue) {
      if (storagesBuilder_ == null) {
        ensureStoragesIsMutable();
        storages_.add(index, builderForValue.build());
        onChanged();
      } else {
        storagesBuilder_.addMessage(index, builderForValue.build());
      }
      return this;
    }
    /**
     * <code>repeated .techschool.pcbook.Storage storages = 7;</code>
     */
    public Builder addAllStorages(
        java.lang.Iterable<? extends protoInJava.Storage> values) {
      if (storagesBuilder_ == null) {
        ensureStoragesIsMutable();
        com.google.protobuf.AbstractMessageLite.Builder.addAll(
            values, storages_);
        onChanged();
      } else {
        storagesBuilder_.addAllMessages(values);
      }
      return this;
    }
    /**
     * <code>repeated .techschool.pcbook.Storage storages = 7;</code>
     */
    public Builder clearStorages() {
      if (storagesBuilder_ == null) {
        storages_ = java.util.Collections.emptyList();
        bitField0_ = (bitField0_ & ~0x00000002);
        onChanged();
      } else {
        storagesBuilder_.clear();
      }
      return this;
    }
    /**
     * <code>repeated .techschool.pcbook.Storage storages = 7;</code>
     */
    public Builder removeStorages(int index) {
      if (storagesBuilder_ == null) {
        ensureStoragesIsMutable();
        storages_.remove(index);
        onChanged();
      } else {
        storagesBuilder_.remove(index);
      }
      return this;
    }
    /**
     * <code>repeated .techschool.pcbook.Storage storages = 7;</code>
     */
    public protoInJava.Storage.Builder getStoragesBuilder(
        int index) {
      return getStoragesFieldBuilder().getBuilder(index);
    }
    /**
     * <code>repeated .techschool.pcbook.Storage storages = 7;</code>
     */
    public protoInJava.StorageOrBuilder getStoragesOrBuilder(
        int index) {
      if (storagesBuilder_ == null) {
        return storages_.get(index);  } else {
        return storagesBuilder_.getMessageOrBuilder(index);
      }
    }
    /**
     * <code>repeated .techschool.pcbook.Storage storages = 7;</code>
     */
    public java.util.List<? extends protoInJava.StorageOrBuilder> 
         getStoragesOrBuilderList() {
      if (storagesBuilder_ != null) {
        return storagesBuilder_.getMessageOrBuilderList();
      } else {
        return java.util.Collections.unmodifiableList(storages_);
      }
    }
    /**
     * <code>repeated .techschool.pcbook.Storage storages = 7;</code>
     */
    public protoInJava.Storage.Builder addStoragesBuilder() {
      return getStoragesFieldBuilder().addBuilder(
          protoInJava.Storage.getDefaultInstance());
    }
    /**
     * <code>repeated .techschool.pcbook.Storage storages = 7;</code>
     */
    public protoInJava.Storage.Builder addStoragesBuilder(
        int index) {
      return getStoragesFieldBuilder().addBuilder(
          index, protoInJava.Storage.getDefaultInstance());
    }
    /**
     * <code>repeated .techschool.pcbook.Storage storages = 7;</code>
     */
    public java.util.List<protoInJava.Storage.Builder> 
         getStoragesBuilderList() {
      return getStoragesFieldBuilder().getBuilderList();
    }
    private com.google.protobuf.RepeatedFieldBuilderV3<
        protoInJava.Storage, protoInJava.Storage.Builder, protoInJava.StorageOrBuilder> 
        getStoragesFieldBuilder() {
      if (storagesBuilder_ == null) {
        storagesBuilder_ = new com.google.protobuf.RepeatedFieldBuilderV3<
            protoInJava.Storage, protoInJava.Storage.Builder, protoInJava.StorageOrBuilder>(
                storages_,
                ((bitField0_ & 0x00000002) != 0),
                getParentForChildren(),
                isClean());
        storages_ = null;
      }
      return storagesBuilder_;
    }

    private protoInJava.Screen screen_;
    private com.google.protobuf.SingleFieldBuilderV3<
        protoInJava.Screen, protoInJava.Screen.Builder, protoInJava.ScreenOrBuilder> screenBuilder_;
    /**
     * <code>.techschool.pcbook.Screen screen = 8;</code>
     * @return Whether the screen field is set.
     */
    public boolean hasScreen() {
      return screenBuilder_ != null || screen_ != null;
    }
    /**
     * <code>.techschool.pcbook.Screen screen = 8;</code>
     * @return The screen.
     */
    public protoInJava.Screen getScreen() {
      if (screenBuilder_ == null) {
        return screen_ == null ? protoInJava.Screen.getDefaultInstance() : screen_;
      } else {
        return screenBuilder_.getMessage();
      }
    }
    /**
     * <code>.techschool.pcbook.Screen screen = 8;</code>
     */
    public Builder setScreen(protoInJava.Screen value) {
      if (screenBuilder_ == null) {
        if (value == null) {
          throw new NullPointerException();
        }
        screen_ = value;
        onChanged();
      } else {
        screenBuilder_.setMessage(value);
      }

      return this;
    }
    /**
     * <code>.techschool.pcbook.Screen screen = 8;</code>
     */
    public Builder setScreen(
        protoInJava.Screen.Builder builderForValue) {
      if (screenBuilder_ == null) {
        screen_ = builderForValue.build();
        onChanged();
      } else {
        screenBuilder_.setMessage(builderForValue.build());
      }

      return this;
    }
    /**
     * <code>.techschool.pcbook.Screen screen = 8;</code>
     */
    public Builder mergeScreen(protoInJava.Screen value) {
      if (screenBuilder_ == null) {
        if (screen_ != null) {
          screen_ =
            protoInJava.Screen.newBuilder(screen_).mergeFrom(value).buildPartial();
        } else {
          screen_ = value;
        }
        onChanged();
      } else {
        screenBuilder_.mergeFrom(value);
      }

      return this;
    }
    /**
     * <code>.techschool.pcbook.Screen screen = 8;</code>
     */
    public Builder clearScreen() {
      if (screenBuilder_ == null) {
        screen_ = null;
        onChanged();
      } else {
        screen_ = null;
        screenBuilder_ = null;
      }

      return this;
    }
    /**
     * <code>.techschool.pcbook.Screen screen = 8;</code>
     */
    public protoInJava.Screen.Builder getScreenBuilder() {
      
      onChanged();
      return getScreenFieldBuilder().getBuilder();
    }
    /**
     * <code>.techschool.pcbook.Screen screen = 8;</code>
     */
    public protoInJava.ScreenOrBuilder getScreenOrBuilder() {
      if (screenBuilder_ != null) {
        return screenBuilder_.getMessageOrBuilder();
      } else {
        return screen_ == null ?
            protoInJava.Screen.getDefaultInstance() : screen_;
      }
    }
    /**
     * <code>.techschool.pcbook.Screen screen = 8;</code>
     */
    private com.google.protobuf.SingleFieldBuilderV3<
        protoInJava.Screen, protoInJava.Screen.Builder, protoInJava.ScreenOrBuilder> 
        getScreenFieldBuilder() {
      if (screenBuilder_ == null) {
        screenBuilder_ = new com.google.protobuf.SingleFieldBuilderV3<
            protoInJava.Screen, protoInJava.Screen.Builder, protoInJava.ScreenOrBuilder>(
                getScreen(),
                getParentForChildren(),
                isClean());
        screen_ = null;
      }
      return screenBuilder_;
    }

    private protoInJava.Keyboard keyboard_;
    private com.google.protobuf.SingleFieldBuilderV3<
        protoInJava.Keyboard, protoInJava.Keyboard.Builder, protoInJava.KeyboardOrBuilder> keyboardBuilder_;
    /**
     * <code>.techschool.pcbook.Keyboard keyboard = 9;</code>
     * @return Whether the keyboard field is set.
     */
    public boolean hasKeyboard() {
      return keyboardBuilder_ != null || keyboard_ != null;
    }
    /**
     * <code>.techschool.pcbook.Keyboard keyboard = 9;</code>
     * @return The keyboard.
     */
    public protoInJava.Keyboard getKeyboard() {
      if (keyboardBuilder_ == null) {
        return keyboard_ == null ? protoInJava.Keyboard.getDefaultInstance() : keyboard_;
      } else {
        return keyboardBuilder_.getMessage();
      }
    }
    /**
     * <code>.techschool.pcbook.Keyboard keyboard = 9;</code>
     */
    public Builder setKeyboard(protoInJava.Keyboard value) {
      if (keyboardBuilder_ == null) {
        if (value == null) {
          throw new NullPointerException();
        }
        keyboard_ = value;
        onChanged();
      } else {
        keyboardBuilder_.setMessage(value);
      }

      return this;
    }
    /**
     * <code>.techschool.pcbook.Keyboard keyboard = 9;</code>
     */
    public Builder setKeyboard(
        protoInJava.Keyboard.Builder builderForValue) {
      if (keyboardBuilder_ == null) {
        keyboard_ = builderForValue.build();
        onChanged();
      } else {
        keyboardBuilder_.setMessage(builderForValue.build());
      }

      return this;
    }
    /**
     * <code>.techschool.pcbook.Keyboard keyboard = 9;</code>
     */
    public Builder mergeKeyboard(protoInJava.Keyboard value) {
      if (keyboardBuilder_ == null) {
        if (keyboard_ != null) {
          keyboard_ =
            protoInJava.Keyboard.newBuilder(keyboard_).mergeFrom(value).buildPartial();
        } else {
          keyboard_ = value;
        }
        onChanged();
      } else {
        keyboardBuilder_.mergeFrom(value);
      }

      return this;
    }
    /**
     * <code>.techschool.pcbook.Keyboard keyboard = 9;</code>
     */
    public Builder clearKeyboard() {
      if (keyboardBuilder_ == null) {
        keyboard_ = null;
        onChanged();
      } else {
        keyboard_ = null;
        keyboardBuilder_ = null;
      }

      return this;
    }
    /**
     * <code>.techschool.pcbook.Keyboard keyboard = 9;</code>
     */
    public protoInJava.Keyboard.Builder getKeyboardBuilder() {
      
      onChanged();
      return getKeyboardFieldBuilder().getBuilder();
    }
    /**
     * <code>.techschool.pcbook.Keyboard keyboard = 9;</code>
     */
    public protoInJava.KeyboardOrBuilder getKeyboardOrBuilder() {
      if (keyboardBuilder_ != null) {
        return keyboardBuilder_.getMessageOrBuilder();
      } else {
        return keyboard_ == null ?
            protoInJava.Keyboard.getDefaultInstance() : keyboard_;
      }
    }
    /**
     * <code>.techschool.pcbook.Keyboard keyboard = 9;</code>
     */
    private com.google.protobuf.SingleFieldBuilderV3<
        protoInJava.Keyboard, protoInJava.Keyboard.Builder, protoInJava.KeyboardOrBuilder> 
        getKeyboardFieldBuilder() {
      if (keyboardBuilder_ == null) {
        keyboardBuilder_ = new com.google.protobuf.SingleFieldBuilderV3<
            protoInJava.Keyboard, protoInJava.Keyboard.Builder, protoInJava.KeyboardOrBuilder>(
                getKeyboard(),
                getParentForChildren(),
                isClean());
        keyboard_ = null;
      }
      return keyboardBuilder_;
    }

    /**
     * <code>double weight_kg = 10;</code>
     * @return Whether the weightKg field is set.
     */
    public boolean hasWeightKg() {
      return weightCase_ == 10;
    }
    /**
     * <code>double weight_kg = 10;</code>
     * @return The weightKg.
     */
    public double getWeightKg() {
      if (weightCase_ == 10) {
        return (java.lang.Double) weight_;
      }
      return 0D;
    }
    /**
     * <code>double weight_kg = 10;</code>
     * @param value The weightKg to set.
     * @return This builder for chaining.
     */
    public Builder setWeightKg(double value) {
      weightCase_ = 10;
      weight_ = value;
      onChanged();
      return this;
    }
    /**
     * <code>double weight_kg = 10;</code>
     * @return This builder for chaining.
     */
    public Builder clearWeightKg() {
      if (weightCase_ == 10) {
        weightCase_ = 0;
        weight_ = null;
        onChanged();
      }
      return this;
    }

    /**
     * <code>double weight_lb = 11;</code>
     * @return Whether the weightLb field is set.
     */
    public boolean hasWeightLb() {
      return weightCase_ == 11;
    }
    /**
     * <code>double weight_lb = 11;</code>
     * @return The weightLb.
     */
    public double getWeightLb() {
      if (weightCase_ == 11) {
        return (java.lang.Double) weight_;
      }
      return 0D;
    }
    /**
     * <code>double weight_lb = 11;</code>
     * @param value The weightLb to set.
     * @return This builder for chaining.
     */
    public Builder setWeightLb(double value) {
      weightCase_ = 11;
      weight_ = value;
      onChanged();
      return this;
    }
    /**
     * <code>double weight_lb = 11;</code>
     * @return This builder for chaining.
     */
    public Builder clearWeightLb() {
      if (weightCase_ == 11) {
        weightCase_ = 0;
        weight_ = null;
        onChanged();
      }
      return this;
    }

    private double priceUsd_ ;
    /**
     * <code>double price_usd = 12;</code>
     * @return The priceUsd.
     */
    @java.lang.Override
    public double getPriceUsd() {
      return priceUsd_;
    }
    /**
     * <code>double price_usd = 12;</code>
     * @param value The priceUsd to set.
     * @return This builder for chaining.
     */
    public Builder setPriceUsd(double value) {
      
      priceUsd_ = value;
      onChanged();
      return this;
    }
    /**
     * <code>double price_usd = 12;</code>
     * @return This builder for chaining.
     */
    public Builder clearPriceUsd() {
      
      priceUsd_ = 0D;
      onChanged();
      return this;
    }

    private int releaseYear_ ;
    /**
     * <code>uint32 release_year = 13;</code>
     * @return The releaseYear.
     */
    @java.lang.Override
    public int getReleaseYear() {
      return releaseYear_;
    }
    /**
     * <code>uint32 release_year = 13;</code>
     * @param value The releaseYear to set.
     * @return This builder for chaining.
     */
    public Builder setReleaseYear(int value) {
      
      releaseYear_ = value;
      onChanged();
      return this;
    }
    /**
     * <code>uint32 release_year = 13;</code>
     * @return This builder for chaining.
     */
    public Builder clearReleaseYear() {
      
      releaseYear_ = 0;
      onChanged();
      return this;
    }

    private com.google.protobuf.Timestamp updatedAt_;
    private com.google.protobuf.SingleFieldBuilderV3<
        com.google.protobuf.Timestamp, com.google.protobuf.Timestamp.Builder, com.google.protobuf.TimestampOrBuilder> updatedAtBuilder_;
    /**
     * <code>.google.protobuf.Timestamp updated_at = 14;</code>
     * @return Whether the updatedAt field is set.
     */
    public boolean hasUpdatedAt() {
      return updatedAtBuilder_ != null || updatedAt_ != null;
    }
    /**
     * <code>.google.protobuf.Timestamp updated_at = 14;</code>
     * @return The updatedAt.
     */
    public com.google.protobuf.Timestamp getUpdatedAt() {
      if (updatedAtBuilder_ == null) {
        return updatedAt_ == null ? com.google.protobuf.Timestamp.getDefaultInstance() : updatedAt_;
      } else {
        return updatedAtBuilder_.getMessage();
      }
    }
    /**
     * <code>.google.protobuf.Timestamp updated_at = 14;</code>
     */
    public Builder setUpdatedAt(com.google.protobuf.Timestamp value) {
      if (updatedAtBuilder_ == null) {
        if (value == null) {
          throw new NullPointerException();
        }
        updatedAt_ = value;
        onChanged();
      } else {
        updatedAtBuilder_.setMessage(value);
      }

      return this;
    }
    /**
     * <code>.google.protobuf.Timestamp updated_at = 14;</code>
     */
    public Builder setUpdatedAt(
        com.google.protobuf.Timestamp.Builder builderForValue) {
      if (updatedAtBuilder_ == null) {
        updatedAt_ = builderForValue.build();
        onChanged();
      } else {
        updatedAtBuilder_.setMessage(builderForValue.build());
      }

      return this;
    }
    /**
     * <code>.google.protobuf.Timestamp updated_at = 14;</code>
     */
    public Builder mergeUpdatedAt(com.google.protobuf.Timestamp value) {
      if (updatedAtBuilder_ == null) {
        if (updatedAt_ != null) {
          updatedAt_ =
            com.google.protobuf.Timestamp.newBuilder(updatedAt_).mergeFrom(value).buildPartial();
        } else {
          updatedAt_ = value;
        }
        onChanged();
      } else {
        updatedAtBuilder_.mergeFrom(value);
      }

      return this;
    }
    /**
     * <code>.google.protobuf.Timestamp updated_at = 14;</code>
     */
    public Builder clearUpdatedAt() {
      if (updatedAtBuilder_ == null) {
        updatedAt_ = null;
        onChanged();
      } else {
        updatedAt_ = null;
        updatedAtBuilder_ = null;
      }

      return this;
    }
    /**
     * <code>.google.protobuf.Timestamp updated_at = 14;</code>
     */
    public com.google.protobuf.Timestamp.Builder getUpdatedAtBuilder() {
      
      onChanged();
      return getUpdatedAtFieldBuilder().getBuilder();
    }
    /**
     * <code>.google.protobuf.Timestamp updated_at = 14;</code>
     */
    public com.google.protobuf.TimestampOrBuilder getUpdatedAtOrBuilder() {
      if (updatedAtBuilder_ != null) {
        return updatedAtBuilder_.getMessageOrBuilder();
      } else {
        return updatedAt_ == null ?
            com.google.protobuf.Timestamp.getDefaultInstance() : updatedAt_;
      }
    }
    /**
     * <code>.google.protobuf.Timestamp updated_at = 14;</code>
     */
    private com.google.protobuf.SingleFieldBuilderV3<
        com.google.protobuf.Timestamp, com.google.protobuf.Timestamp.Builder, com.google.protobuf.TimestampOrBuilder> 
        getUpdatedAtFieldBuilder() {
      if (updatedAtBuilder_ == null) {
        updatedAtBuilder_ = new com.google.protobuf.SingleFieldBuilderV3<
            com.google.protobuf.Timestamp, com.google.protobuf.Timestamp.Builder, com.google.protobuf.TimestampOrBuilder>(
                getUpdatedAt(),
                getParentForChildren(),
                isClean());
        updatedAt_ = null;
      }
      return updatedAtBuilder_;
    }
    @java.lang.Override
    public final Builder setUnknownFields(
        final com.google.protobuf.UnknownFieldSet unknownFields) {
      return super.setUnknownFields(unknownFields);
    }

    @java.lang.Override
    public final Builder mergeUnknownFields(
        final com.google.protobuf.UnknownFieldSet unknownFields) {
      return super.mergeUnknownFields(unknownFields);
    }


    // @@protoc_insertion_point(builder_scope:techschool.pcbook.Laptop)
  }

  // @@protoc_insertion_point(class_scope:techschool.pcbook.Laptop)
  private static final protoInJava.Laptop DEFAULT_INSTANCE;
  static {
    DEFAULT_INSTANCE = new protoInJava.Laptop();
  }

  public static protoInJava.Laptop getDefaultInstance() {
    return DEFAULT_INSTANCE;
  }

  private static final com.google.protobuf.Parser<Laptop>
      PARSER = new com.google.protobuf.AbstractParser<Laptop>() {
    @java.lang.Override
    public Laptop parsePartialFrom(
        com.google.protobuf.CodedInputStream input,
        com.google.protobuf.ExtensionRegistryLite extensionRegistry)
        throws com.google.protobuf.InvalidProtocolBufferException {
      Builder builder = newBuilder();
      try {
        builder.mergeFrom(input, extensionRegistry);
      } catch (com.google.protobuf.InvalidProtocolBufferException e) {
        throw e.setUnfinishedMessage(builder.buildPartial());
      } catch (com.google.protobuf.UninitializedMessageException e) {
        throw e.asInvalidProtocolBufferException().setUnfinishedMessage(builder.buildPartial());
      } catch (java.io.IOException e) {
        throw new com.google.protobuf.InvalidProtocolBufferException(e)
            .setUnfinishedMessage(builder.buildPartial());
      }
      return builder.buildPartial();
    }
  };

  public static com.google.protobuf.Parser<Laptop> parser() {
    return PARSER;
  }

  @java.lang.Override
  public com.google.protobuf.Parser<Laptop> getParserForType() {
    return PARSER;
  }

  @java.lang.Override
  public protoInJava.Laptop getDefaultInstanceForType() {
    return DEFAULT_INSTANCE;
  }

}

